...
import com.buzz.vpn.Application;
import com.buzz.vpn.R;
import com.buzz.vpn.UserDefaults;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal, SocketProtect.IProtectSocket {
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel, long when, ConnectionStatus status, Intent intent) {
        ...
        if (mProfile != null) nbuilder.setContentTitle(getString(R.string.notifcation_title));
...
//            return;
    ...
    private void startTimer() {
        ...
        if (userDefaults.getUserType().equals("3")) {
            if (userDefaults.isFreeMinEnabled() && userDefaults.getFreeMin() > 0) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if (defaults.getFreeMin() == 0) {
                    ...
                    long freeMin = defaults.getFreeMin() - 1;
                    defaults.setFreeMin(freeMin);
                    defaults.save();
                    disconnectHandler.postDelayed(this, 60 * 1000 - SystemClock.elapsedRealtime() % 1000);
                }
            }
        };
        disconnectHandler.post(runnable);
        startWakeLock();
    }
    ...
    private void updateFreeVPNMinute() {
/*        UserDefaults defaults = new UserDefaults(getApplicationContext());
        defaults.setFreeMinPending(true);
        defaults.save();
        StringRequest request = new StringRequest(Request.Method.POST, defaults.getFreeMinUrl(),
                response -> {
                    try{
                        JSONObject resultObject = new JSONObject(response);
                        String message = resultObject.getString("message").trim();
                        JSONArray ipBundle = new JSONArray(Application.getInstance().getDecryptedResponse(resultObject.getString("ip_bundle")));
                        if(message.equalsIgnoreCase("Updated")){
                            defaults.setFreeMinPending(false);
                            if (ipBundle.length() > 0){
                                defaults.setServerList(ipBundle.toString());
                            }
                            defaults.save();
                ...
                Throwable::printStackTrace)
        {
            ...
            protected Map<String,String> getParams(){
                ...
                params.put("username", defaults.getUserName());
                params.put("setFreeMinute", String.valueOf(defaults.getFreeMin()));
                params.put("pass", defaults.getPassword());
                params.put("udid", defaults.getUdid());
                params.put("countryCode", CountryUtils.getDeviceCountryCode(getApplicationContext()));
                params.put("vpnAppVersion", Application.getInstance().getVersion());
                ...
                params.put("isRooted", Application.getInstance().getRootedState());
                params.put("bundle2", "1");
                return params;
            }
            @Override
