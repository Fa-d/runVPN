...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public static boolean isEmbedded(String data) {
        ...
        if (data.startsWith(INLINE_TAG) || data.startsWith(DISPLAYNAME_TAG)) return true;
        else return false;
    }
        ...
        switch (mProfileVersion) {
            ...
            case 6:
                for (Connection c : mConnections)
                    if (c.mProxyType == null) c.mProxyType = Connection.ProxyType.NONE;
            case 7:
                if (mAllowAppVpnBypass) mBlockUnusedAddressFamilies = !mAllowAppVpnBypass;
            case 8:
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        if (mConnections.length == 1) {
            ...
            if (mRemoteRandom) cfg.append("remote-random\n");
        ...
        if (isUserPWAuth()) {
            if (mAuthRetry == AUTH_RETRY_NOINTERACT) cfg.append("auth-retry nointeract\n");
        }
        ...
        if (mUseTLSAuth) {
            ...
            else if (useTlsCrypt) cfg.append(insertFileData("tls-crypt", mTLSAuthFilename));
            else if (useTlsCrypt2) cfg.append(insertFileData("tls-crypt-v2", mTLSAuthFilename));
            else cfg.append(insertFileData("tls-auth", mTLSAuthFilename));
        ...
        if (mUsePull && mRoutenopull) cfg.append("route-nopull\n");
        ...
        if (mUseDefaultRoutev6) cfg.append("route-ipv6 ::/0\n");
        ...
        if (mMssFix != 0) {
            if (mMssFix != 1450) {
                ...
                else cfg.append(String.format(Locale.US, "mssfix %d\n", mMssFix));
            } else cfg.append("mssfix\n");
        }
        ...
        if (mAuthenticationType != TYPE_STATICKEYS) {
            ...
            if (mExpectTLSCert) cfg.append("remote-cert-tls server\n");
        }
        ...
        if (mUseRandomHostname) cfg.append("#my favorite options :)\nremote-random-hostname\n");

        if (mUseFloat) cfg.append("float\n");
        ...
        if (mPushPeerInfo) cfg.append("push-peer-info\n");
    ...
    public String getPlatformVersionEnvString() {
        return String.format(Locale.US, "%d %s %s %s %s %s", Build.VERSION.SDK_INT, Build.VERSION.RELEASE, NativeUtils.getNativeAPI(), Build.BRAND, Build.BOARD, Build.MODEL);
    }
...
//        }
    ...
    public int checkProfile(Context context, boolean useOpenVPN3) {
        ...
        for (Connection c : mConnections) {
            if (c.mProxyType == Connection.ProxyType.ORBOT) {
                if (usesExtraProxyOptions()) return R.string.error_orbot_and_proxy_options;
    ...
    public boolean requireTLSKeyPassword() {
        ...
        else if (data.contains("-----BEGIN ENCRYPTED PRIVATE KEY-----")) return true;
        else return false;
    }
    ...
    public String getSignedData(Context c, String b64data, boolean pkcs1padding) {
        ...
        byte[] signed_bytes;
        if (mAuthenticationType == TYPE_EXTERNAL_APP) signed_bytes = getExtAppSignedData(c, data);
        else signed_bytes = getKeyChainSignedData(data, pkcs1padding);

        if (signed_bytes != null) return Base64.encodeToString(signed_bytes, Base64.NO_WRAP);
        else return null;
    }

    private byte[] getExtAppSignedData(Context c, byte[] data) {
        if (TextUtils.isEmpty(mExternalAuthenticator)) return null;
        try {
    ...
    private byte[] getKeyChainSignedData(byte[] data, boolean pkcs1padding) {
        ...
        try {
            @SuppressLint("GetInstance") String keyalgorithm = privkey.getAlgorithm();
             ...
             */
                Cipher signer;
                if (pkcs1padding) signer = Cipher.getInstance("RSA/ECB/PKCS1PADDING");
                else signer = Cipher.getInstance("RSA/ECB/NoPadding");
    ...
    private boolean usesExtraProxyOptions() {
        if (mUseCustomConfig && mCustomConfigOptions != null && mCustomConfigOptions.contains("http-proxy-option "))
            return true;
        for (Connection c : mConnections)
            if (c.usesExtraProxyOptions()) return true;
